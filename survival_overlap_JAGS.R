model
{
  ...
  {
    beta1[j] ~ dnorm(mubeta[1] + quantile[j]*beta.t[1], tau[1]) # quantile
    beta2[j] ~ dnorm(mubeta[2] + quantile[j]*beta.t[2], tau[2]) # conspecific nci effect
    beta3[j] ~ dnorm(mubeta[3], tau[3]) # heterospecific nci effect
    beta4[j] ~ dnorm(mubeta[4], tau[4]) # overlap effect
