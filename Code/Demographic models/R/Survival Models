...
data_survival <- read.csv("survival_model_data.csv")

#Create column for log transformation of neighborhood range overlap or SES_range_overlap
data_survival <- mutate(data_survival, logSES_range_overlap = log(SES_range_overlap)) %>%
...
mod1 <- jags.model("survival_geography_traits_JAGS.R", data=data_mod1, n.chains=6, n.adapt=n.adapt)
...
mod2 <- jags.model("survival_geography_JAGS.R", data=data_mod2, n.chains=6, n.adapt=n.adapt)
...
mod3 <- jags.model("survival_traits_JAGS.R", data=data_mod3, n.chains=6, n.adapt=n.adapt)
